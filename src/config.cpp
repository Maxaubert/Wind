...
Config LoadConfig(const std::wstring& path) {
    ...
    if (!f) {
        ...
        out << "; Wind magnifier config. Edit and save; changes apply within ~1s.\n"
               ...
               "multiMonitor=0\n"
               "; cropCapture (opt-in, captureCopy=1 only): 0=always copy all changed regions (cache\n"
               ";   never stale, default); 1=on a full-screen repaint (games) copy only the magnified\n"
               ";   region (cuts 4K HDR GPU copy ~zoom^2) but screen edges can briefly show a previous\n"
               ";   window after a switch. Has no effect when captureCopy=0 (zero-copy path).\n"
               "cropCapture=0\n"
