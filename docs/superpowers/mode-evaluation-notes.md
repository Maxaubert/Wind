...
| 1 | `lowPower=1` | Magnification API (`MagSetFullscreenTransform`) | **great on desktop, bad in games** | Re-test: smooth + cursor LOCKED on the desktop, ~1% GPU (excellent - lower than the earlier 2-3% estimate). No crash this run (earlier crash likely unrelated). BUT in a game, FPS drops to ~half: the public fullscreen transform forces the game off direct-scanout through DWM composition (vsync-locked -> halves on a missed vblank). Windows Magnifier avoids this via private DWM interfaces unavailable to the public API. NOTE the earlier wobble complaint did not recur here - Mag feels smooth/locked at this zoom. Fix for the game-FPS issue = adaptive (mode 2). |
