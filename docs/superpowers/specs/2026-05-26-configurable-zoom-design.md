...
**Smooth mode** adds an acceleration multiplier to the zoom-*in* rate only:
...
accelMult = 1.0                                              if linear mode
          = 1 + (smoothZoomAccel - 1) * min(heldIn / smoothZoomRamp, 1)   if smooth mode

effective speedIn = zoomInSpeed * accelMult
```

So `accelMult` ramps **linearly from 1 to `smoothZoomAccel`** over `smoothZoomRamp` seconds of
continuous holding, then stays at `smoothZoomAccel`. The zoom-in rate therefore climbs from
`zoomInSpeed` (the slow start) up to `zoomInSpeed * smoothZoomAccel` (the fast steady rate).

- The "slow start" comes from `zoomInSpeed` (the start/base rate); the climb comes from
  `smoothZoomAccel`. No separate "start fraction" knob.
...
|---|---|---|---|
| `smoothZoom` | `0` | 0 / 1 | 0 = linear (current); 1 = accelerate zoom-in |
| `zoomInSpeed` | `1.0` | 0.25 - 4.0 | zoom-in rate multiplier (both modes; the *start* rate in smooth) |
| `zoomOutSpeed` | `1.0` | 0.25 - 4.0 | zoom-out rate multiplier (both modes) |
| `smoothZoomAccel` | `3.0` | 1.0 - 8.0 | smooth-mode top rate = start x this (1 = no accel) |
| `smoothZoomRamp` | `0.6` | 0.1 - 3.0 (seconds) | seconds of holding to reach the top rate |
...
- **Smooth ramp:** with `smoothZoom=1`, at `heldIn=0` the in-rate equals `zoomInSpeed` (accelMult 1);
  at `heldIn >= smoothZoomRamp` it equals `zoomInSpeed * smoothZoomAccel` (accelMult = accel); halfway
  is the linear midpoint. Plateaus (does not exceed accel) past the ramp.
